...
def prepare_ligand(input_file:str):
    """
    ...
    # Get output filename
    output_file = os.path.join('Examples', f"{os.path.splitext(os.path.basename(input_file))[0]}.pdbqt")
...
def main():
    ...
    input_ligand = os.path.join('Examples', input_filename)

    try:
        # Clean file and convert to PDBQT
        output_ligand = prepare_ligand(input_ligand)
